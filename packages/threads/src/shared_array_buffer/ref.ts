...
abstract class WASIFarmRefUseArrayBufferBase extends WASIFarmRef {
  ...
  protected call_fd(
    fd: number,
    start_callback: (data: ViewSet<SharedArrayBuffer, 64>) => void,
  ...
  protected call_double_fd(
    fd1: number,
    fd2: number,
    callback: (data: ViewSet<SharedArrayBuffer, 64>) => void,
    finished_callback?: undefined,
  ): number;
  ...
  private call_fd_func<T>(
    ...
    return this.get_fd_caller(fd).call_and_wait_blocking(
      start_callback,
      (data) => {
        const errno_offset = data.u32.length - 1;
        const errno = data.i32[errno_offset];
        if (!finished_callback) return errno;
        return finished_callback(data, errno);
      },
    );
  }
}
